...
module.exports = class CatBox {
	...
	async createAlbum({
		...
		try {
			const response = await this._fetch({
				...
				title,
				desc: description,
				files: files.length ? Resolver.resolveFiles(files).join(' ') : ''
			})
	...
	async editAlbum({
		...
		try {
			if (short) {
				const album = await this.getAlbum(Resolver.resolveAlbum(short))
				const response = await this._fetch({
					reqtype: 'editalbum',
					userhash: this.userHash,
					short: album.short,
					...
					files: Resolver.resolveFiles(album.files.split(' '), files).join(' ')
				})
		...
		try {
			if (short && files.length) {
				const response = await this._fetch({
					...
					userhash: this.userHash,
					short: Resolver.resolveAlbum(short),
					files: Resolver.resolveFiles(files).join(' ')
				})
