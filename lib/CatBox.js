...
module.exports = class CatBox {
	...
	async delete(...files) {
		...
		try {
			files = Resolver.resolveFiles(files)
			const response = await this._fetch({
				reqtype: 'deletefiles',
				userhash: this.userHash,
				files: files.join(' ')
	...
	async getAlbum(short) {
		try {
			const response = await this._fetch({
				reqtype: 'getalbum',
				short: Resolver.resolveAlbum(short)
			})
	...
	async createAlbum(options = {}) {
		try {
			let files = options.files ? Resolver.resolveFiles(options.files) : null
			const response = await this._fetch({
				reqtype: 'createalbum',
				userhash: this.userHash,
				title: options.title || '',
	...
	async editAlbum(options = {}) {
		...
		try {
			if (options.short) {
				const album = await this.getAlbum(Resolver.resolveAlbum(options.short))
				const response = await this._fetch({
					reqtype: 'editalbum',
					userhash: this.userHash,
					short: album.short,
					...
					files: Resolver.resolveFiles(album.files.split(' '), options.files).join(' ')
				})
		...
		try {
			if (options.short && options.files) {
				const response = await this._fetch({
					reqtype: 'addtoalbum',
					userhash: this.userHash,
					short: Resolver.resolveAlbum(options.short),
					files: Resolver.resolveFiles(options.files).join(' ')
				})
	...
	async _fetch(data) {
		...
		Object.entries(data).filter(([key, value]) => value !== null).map(([key, value]) => FormData.append(key, value))
